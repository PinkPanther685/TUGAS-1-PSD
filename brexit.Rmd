...
library(dplyr)
library(scales)
library(stringr)  # Tambahan untuk str_wrap()

ggplot(brexit %>%
         group_by(region, opinion) %>%
         summarise(count = n(), .groups = 'drop') %>%
         mutate(proportion = count / sum(count)),
       aes(x = opinion, y = proportion, fill = opinion)) +
  geom_bar(stat = "identity") +
  facet_wrap(~region, nrow = 1, labeller = label_wrap_gen(width = 15)) +  # Lebar teks lebih besar
  ...
  theme(
    ...
    strip.text = element_text(size = 10)  # Menyesuaikan ukuran teks facet
  )
```
...
library(ggplot2)
library(dplyr)
library(scales) # Memuat package untuk meningkatkan label sumbu

ggplot(brexit %>%
         group_by(region, opinion) %>%
         summarise(count = n(), .groups = 'drop') %>%
         mutate(proportion = count / sum(count)),
       aes(x = region, y = proportion, fill = opinion)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) + # Menggunakan dodging untuk memisahkan opinion
  scale_y_continuous(labels = scales::percent_format()) + # Mengubah skala sumbu y menjadi persen
  guides(fill = guide_legend(title = "Opinion")) + # Memperbaiki legenda
  labs(
    title = "Was Britain right/wrong to vote to leave EU?",
    subtitle = "YouGov Survey Results, 2-3 September 2019",
    caption = "Source: bit.ly/2lCJZVg",
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1) # Memutar label sumbu x agar lebih terbaca
  )
```
